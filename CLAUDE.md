...
Real, localized app screenshots are shown via `<AppScreenshot scene="…" />` (`src/components/AppScreenshot.astro`). Don't reinvent this per page. **Place a shot only where it makes the page's point** — a screenshot has to be *needed*, not just available. Today they live solely on the **`/voice/` product page**: the hero (`in-call`, proving "recorded and transcribed") and one per how-it-works step (`add-account` → `incoming-call` → `history`). The ecosystem homepage and the use-cases page deliberately carry none — a shot there was decoration, not proof. Only the scenes actually rendered are committed/synced; don't add a scene to the sync list without a usage to go with it.
