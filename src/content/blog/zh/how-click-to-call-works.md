...
解析器之后还有 1 秒防抖，因为某些桌面环境会为一次点击触发两次处理程序，也因为网页不该能靠连发一串 `tel:` 打开来刷屏拨号面板。而守护进程自己的拨号校验仍然拥有最终发言权——任何不可路由的东西它都会返回 `400`。这道校验位于 `POST /calls/dial`，和[内置命令行工具](/zh/blog/place-calls-from-the-command-line/)拨号所用的是同一个端点，所以一个被点击的链接受到的审查，和一次脚本拨号完全一样。解析器是第一道闸，不是唯一一道。
...
点击拨号随 [0.0.43](/zh/voice/changelog/#0.0.43) 推出，同一个版本也让 WaveKat Voice 学会了[用来电流程接听来电](/zh/blog/answer-calls-with-a-call-flow/)——正是上面这一切在「打进来」方向上的对应物。和通话路径上的其他一切一样，它跑在[我们自研的 SIP 引擎](/zh/blog/our-own-sip-engine/)之上——这也正是我们能把一次点击一路推理到数据包的原因。
